...
public class Point extends AngularPosition implements Shape {
  ...
  protected final void checkCoordinatesInGeocentric(final double longitudeVal, final double latitudeVal) {
    ...
    if (longitudeVal > MAX_LONG || longitudeVal < MIN_LONG) {
      final String message = String.format("long=%s - longitude must be included in the following range [-180 180]\n", longitudeVal);
      errorMessage = errorMessage.append(message);
    }
    if (latitudeVal > MAX_LAT || latitudeVal < MIN_LAT) {
      final String message = String.format("lat=%s - latitude must be included in the following range [-90 90]\n", latitudeVal);
      errorMessage = errorMessage.append(message);
    }
  ...
  protected final void checkCoordinatesInEquatorial(final double longitudeVal, final double latitudeVal) {
    ...
    if (longitudeVal > MAX_RA || longitudeVal < MIN_RA) {
      final String message = String.format("RA=%s - right ascension must be included in the following range [0 360]\n", longitudeVal);
      errorMessage = errorMessage.append(message);
    }
  ...
  protected final void setPoint(final double longitudeVal, final double latitudeVal, final CoordSystem coordSystemVal) {
    ...
    switch (coordSystemVal) {
      ...
      case GALACTIC:
        ...
        break;
      case GEOCENTRIC:
        double longitudeValTmp;
        if (longitudeVal == MAX_LONG) {
          longitudeValTmp = MAX_LONG - NUMERICAL_PRECISION;
        } else if (longitudeVal == MAX_LONG * -1) {
          longitudeValTmp = MAX_LONG * -1 + NUMERICAL_PRECISION;
        } else {
          longitudeValTmp = longitudeVal;
        }
        checkCoordinatesInGeocentric(longitudeValTmp, latitudeVal);
        setTheta(Math.toRadians(CoordSystem.convertLatitudeGeoToTheta(latitudeVal)));
        setPhi(Math.toRadians(CoordSystem.convertLongitudeGeoToPhi(longitudeValTmp)));
        break;
      default:
  ...
  public final boolean equals(final Object obj) {
    ...
    if (this == obj) {
      ...
      isEqual = that.getLatitude() == this.latitude
