 ...
 */
public class OpenSearchVOSiaDescription extends SitoolsParameterizedResource {

  /**
   * Logger.
   */
  private static final Logger LOG = Logger.getLogger(OpenSearchVOSiaDescription.class.getName());
  /**
   * Data model.
   */
  private Map dataModel = new HashMap();
  /**
   * Data model for the configuration.
   */
  ...
  public final void doInit() {
    super.doInit();
    parameters = ((OpenSearchVOSiaSearchApplicationPlugin) getApplication()).getModel().getParametersMap();
  }
  ...
  private String buildTemplateURL() throws JSONException, IOException {
    final String serviceURL = getSitoolsSetting("Starter.PUBLIC_HOST_DOMAIN") + ((OpenSearchVOSiaSearchApplicationPlugin) getApplication()).getModel().getUrlAttach();
    String queryShape = parameters.get("queryShape").getValue();
    String shape;
    if (queryShape.equals(GeometryShape.HEALPIX.getShape())) {
