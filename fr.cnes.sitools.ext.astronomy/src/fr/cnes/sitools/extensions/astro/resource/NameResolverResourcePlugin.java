 ...
 *
 * <p>
 * The parameters are coordinates system and object name for deep sky objects
 * and stars. The parameters are coordinates system, solar object name and time
 * for solar objects </p>
 *
 * <p>
 * This service answers to the following scenario:<br/> As user, I want to set
 * the name and the service returns its coordinates in order to integrate this
 * service in a tool. Moreover several output can be supported. This means it is
 * possible to get several coordinates for on single name.
 * <br/>
 ...
 * @startuml NameResolver-usecase.png title Name Resolver User --> (Name
 * Resolver) : requests Admin --> (Name Resolver) : adds and configures the Name
 * Resolver. (Name Resolver) .. (project) : uses
 * @enduml
 * @startuml package "Services" { HTTP - [NameResolverResourcePlugin] [Cache] }
 * cloud { [CDS] [IMCCE] [IAS] } package "Project/Dataset" { HTTP -
 * [Project/Dataset] } [NameResolverResourcePlugin] --> [Project/Dataset] :
 * "attached to" [NameResolverResourcePlugin] --> [CDS] : "uses"
 * [NameResolverResourcePlugin] --> [IMCCE] : "uses"
 * [NameResolverResourcePlugin] --> [IAS] : "uses" [NameResolverResourcePlugin]
 * .. [Cache]
 * @enduml
 */
