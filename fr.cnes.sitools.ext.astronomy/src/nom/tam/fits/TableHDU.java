...
public abstract class TableHDU extends BasicHDU {
  ...
  /**
   * Add a row to the end of the table. If this is the first row, then this will add appropriate columns for each of the
   * entries.
   */
  ...
  /**
   * Specify column metadata for a given column in a way that allows all of the column metadata for a given column to be
   * organized together.
   * 
   * @param index
   ...
   * @param after
   *          Should the header card be after the current column metadata block (true), or immediately before the TFORM
   *          card (false).
   * @throws FitsException
   */
   ...
   *          The 0-based index of the column
   * @param after
   *          True if the cursor should be placed after the existing column metadata or false if the cursor is to be
   *          placed before the TFORM value. If no corresponding TFORM is found, the cursoe will be placed at the end of
   *          current header.
   */
   ...
   * but re-implemented using the DataTable and changes to AsciiTable so that it can be done easily for both Binary and
   * ASCII tables.
   * 
   * @param row
   ...
   * 
   * @param firstRow
   *          the (0-based) index of the first row to be deleted. This is zero-based indexing: 0<=firstrow< number of
   *          rows.
   * @param nRow
  ...
  public void deleteColumnsIndexZero(int column, int len, String[] fields) throws FitsException {
    ...
    if (column < 0 || len < 0 || column + len > getNCols()) {
      throw new FitsException("Illegal columns deletion request- Start:" + column + " Len:" + len + " from table with "
          + getNCols() + " columns");
    }
  ...
  /**
   * Get the stems of the keywords that are associated with table columns. Users can supplement this with their own and
   * call the appropriate deleteColumns fields.
   */
